...
def main():
    i=1
    annotation_file, PATH = download_and_prep_images()
    img_name_vector, train_captions = limit_database_size(annotation_file, PATH)
    ifem=proces_with_InceptionV3(img_name_vector)
    # Choose the top top_k words from the vocabulary
    top_k = 5000
    cap_vector, tokenizer, max_length = tokenize_captions(top_k, train_captions)
    img_name_train, cap_train, img_name_val, cap_val = split_data_training_testing(img_name_vector, cap_vector)
    dataset, encoder, decoder, optimizer, loss_object, num_steps, afs = create_tf_training_dataset(top_k, img_name_train, cap_train)
    start_epoch = 0
    start_epoch, ckpt_manager = checkpoint(start_epoch, encoder, decoder, optimizer)
    
    #### Training ####
    EPOCHS = 20
    loss_plot = epoch_loop(start_epoch, EPOCHS, dataset, decoder, encoder, optimizer, tokenizer, loss_object, num_steps, ckpt_manager)
    #plot_loss(loss_plot)
    validation_set_captions(img_name_val, cap_val, tokenizer, max_length, afs, decoder, encoder,ifem)
    ### Try out your own images
    #image_url = 'https://tensorflow.org/images/surf.jpg'
    image_url = 'https://www.gstatic.com/webp/gallery3/1.png'
    tryout(image_url, max_length, afs, decoder, encoder, ifem, tokenizer)
    print('endofmain')
...
def epoch_loop(start_epoch, EPOCHS, dataset, decoder, encoder, optimizer, tokenizer, loss_object, num_steps, ckpt_manager):
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        for (batch, (img_tensor, target)) in enumerate(dataset):
            batch_loss, t_loss = train_step(img_tensor, target, decoder, encoder, optimizer, tokenizer, loss_object)
            total_loss += t_loss
...
def tryout(image_url, max_length, afs, decoder, encoder, ifem, tokenizer):
    #### Try it on your own images ####
    image_extension = image_url[-4:]
    image_path = tf.keras.utils.get_file('image'+image_extension,
                                         origin=image_url)

    result, attention_plot = evaluate(image_path, max_length, afs, decoder, encoder, ifem, tokenizer)
    print ('Prediction Caption:', ' '.join(result))
